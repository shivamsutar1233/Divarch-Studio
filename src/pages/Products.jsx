...
export default function Products() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const SHEET_RANGE = "A2:AI"; // Full range including all product information
      ...
      console.log("Raw data:", data.table.rows[0]?.c); // Log first row to see structure

      const formattedProducts = data.table.rows.map((row) => {
        const regularPrice = row.c[5]?.v || 0;
        // Check for sale price specifically in column M (index 12)
        // console.log("Checking sale price in column M:", row.c[12].v);
        const salePrice =
